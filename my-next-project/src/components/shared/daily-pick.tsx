...
const DailyPick = async ({ data }: any) => {
  ...
  const queryResult = await axios.get(data.config.query.url, {
    baseURL: "https://tl1-www.guitarcenter.com/rest/model",
    params: data.config.query.params,
  });
  ...
  if (emData?.emPageData?.Section) {
    ...
    const formatData = {
      ...
      marketingImageSrc: data.config.marketingImageSrc,
      marketingMobileImageSrc: data.config.marketingMobileImageSrc,
      marketingTabletImageSrc: data.config.marketingTabletImageSrc,
      featuredDealsLink: data.config.featuredDealsLink || "/Daily-Pick.gc?icid=LP6669",
